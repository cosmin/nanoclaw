...
Adds Parallel AI MCP integration to MicroClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses MicroClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. MicroClaw already set up and running
...
Add Parallel AI MCP servers after the microclaw server:
```typescript
const mcpServers: Record<string, any> = {
  microclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__microclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__microclaw__send_message
   4. Use mcp__microclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | container run -i --entrypoint /bin/echo microclaw-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/microclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.microclaw`
