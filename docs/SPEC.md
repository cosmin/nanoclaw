...
│  │    • mcp__microclaw__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── microclaw.log               # Host stdout
│   └── microclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.microclaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'microclaw-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your microclaw installation
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: microclaw (scheduler)
   │
   ▼
...
Claude: [calls mcp__microclaw__schedule_task]
        {
...
The `microclaw` MCP server is created dynamically per agent call with the current group's context.
...
1. **Ensures Apple Container system is running** - Automatically starts it if needed; kills orphaned MicroClaw containers from previous runs
...
### Service: com.microclaw

**launchd/com.microclaw.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/microclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/microclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.microclaw.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep microclaw` |
| "Claude Code process exited with code 1" | Apple Container failed to start | Check logs; MicroClaw auto-starts container system but may fail |
...
- `logs/microclaw.log` - stdout
- `logs/microclaw.error.log` - stderr
